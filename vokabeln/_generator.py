...
CARDS = [
    ...
    # === Klasse 20.05.2026 — Valencia ===
    # Wohnen / Roommates
    ("Colega de quarto", "Mitbewohner/in", "Roommate"),
    ("Quarto", "Zimmer", ""),
    ("Inscrever-se", "sich anmelden / einschreiben", "z.B. ginásio, curso — reflexiv"),
    ("Tu inscreves-te", "du meldest dich an", "inscrever-se konjugiert"),
    ("Eu inscrevo-me", "ich melde mich an", ""),
    ("Localização", "Lage / Standort", "Estou numa boa localização"),
    ("Coworking", "Coworking-Space", ""),
    ("Mesa", "Tisch", "auch im Coworking"),
    ("Área aberta", "offener Bereich", ""),
    # Reisen / Konferenzen
    ("Convenção / Conferência", "Konvention / Konferenz", "convenção de tatuagem"),
    ("Visitei", "ich besuchte", "visitar Pretérito"),
    ("Eu vim para...", "Ich kam nach...", "vir Pretérito"),
    # Konversation
    ("Como funciona?", "Wie funktioniert es?", ""),
    ("Como vai?", "Wie geht es?", "Como vai o seu projecto?"),
    ("Como vão as coisas?", "Wie laufen die Dinge?", ""),
    ("Eu acho que...", "Ich denke, dass...", ""),
    ("Acho que a vida é assim", "Das Leben ist halt so", "philosophisch"),
    ("Me deixa", "Lass mich / macht mich", "deixar = lassen"),
    ("Parece", "es scheint / sieht aus", "parecer"),
    ("Parecido com", "ähnlich wie", "Komparativ-Konstruktion"),
    ("Comparado com", "verglichen mit", ""),
    ("Mesma língua", "gleiche Sprache", "mesmo/mesma"),
    ("Por isso", "deshalb", ""),
    ("Primeiro... depois...", "zuerst... dann...", "Sequenz"),
    # Komparativ
    ("Mais X do que Y", "mehr X als Y", "Komparativ-Standard"),
    ("Mais caro do que Lisboa", "teurer als Lissabon", ""),
    ("Saiu caro / Saiu barato", "es kam teuer / billig (raus)", "sair de preço"),
    # Konditional
    ("Seria", "es wäre", "Konditional von ser"),
    ("Gostaria de...", "Ich würde gerne...", "Konditional - höflich"),
    # Verben Pretérito (neu/wichtig)
    ("Eu pensei", "ich dachte", "pensar Pretérito"),
    ("Eu percebi", "ich verstand / realisierte", "perceber"),
    ("Eu entendi", "ich verstand", "entender"),
    ("Eu fiz pesquisa", "ich habe recherchiert", "fazer pesquisa"),
    # Vokabeln
    ("Pesquisa", "Recherche / Suche", ""),
    ("Plano", "Plan", "Meu plano é..."),
    ("Projecto", "Projekt", "PT-PT mit -ct! (BR: projeto)"),
    ("Sozinho / Sozinha", "allein", "Estou sozinho"),
    ("Ninguém", "niemand", "Não conheço ninguém"),
    ("Algumas / Alguns", "einige", "Algumas pessoas"),
    ("Clube de corrida", "Lauftreff", ""),
    ("Carpinteiro / Carpinteira", "Zimmermann / Zimmerin", "Handwerker"),
    ("Ainda", "noch / immer noch", "Ainda não fiz isso"),
    ("Ainda não", "noch nicht", ""),
    # Ausdrücke
    ("Amo rotina", "Ich liebe Routine", "amar = lieben"),
    ("Mudam-se para Espanha", "Sie ziehen nach Spanien um", "mudar-se 3.Pl"),
    ("Ele vai sair / se mudar", "Er wird ausziehen / umziehen", ""),
    ("Tu vais ficar feliz", "Du wirst glücklich werden", "ficar = werden"),
    ("Vou ficar 2 meses", "Ich werde 2 Monate bleiben", "ficar = bleiben"),
    ("No fim dos dois meses", "am Ende der zwei Monate", ""),
